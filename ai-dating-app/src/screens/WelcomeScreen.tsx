...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onStart, onLogin, onGoogleAuth, apiBaseUrl }) => {
  ...
  // Resolved asynchronously: the device has to be asked whether Apple auth works.
  ...
  const signInWithGoogleToken = async (idToken: string) => {
    ...
    try {
      ...
      // One account per phone is a refusal, not a sign-in failure. Google
      // already succeeded by this point, so it gets its own dialog.
  ...
  const startAppleAuth = async () => {
    if (appleLoading || !apiBaseUrl || !onGoogleAuth) return;
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.neonGreen }]}>
      ...
      <View style={styles.content}>
        ...
        >
          ...
          {/* Sign in with Apple. iOS only, and required there because Google is
              offered alongside it (App Store guideline 4.8). */}
          {appleAvailable && onGoogleAuth ? (
